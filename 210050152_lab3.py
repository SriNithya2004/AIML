...
class LinearRegressionBatchGD:
    ...
    def compute_gradient(self, X, y, weights):
        ...
        # # TODO add the gradient descent update of w arising from the regularizer
        # return dw
        
        # Compute the prediction y_pred
        y_pred = X @ weights

        # Compute the gradient descent update of w arising from the loss
        difference = y_pred - y
        loss_gradient = (2*(X.T @ difference)) / X.shape[0]

        # Compute the gradient descent update of w arising from the regularizer
        if self.regularization == 'none':
            reg_gradient = np.zeros_like(weights)
        elif self.regularization == 'l1':
            reg_gradient = self.reg_strength * np.sign(weights)
        elif self.regularization == 'l2':
            reg_gradient = 2 * self.reg_strength * weights
        elif self.regularization == 'ElasticNet':
            alpha = self.reg_strength[1]
            l1_reg = self.reg_strength[0] * alpha * np.sign(weights)
            l2_reg = 2 * self.reg_strength[0] * (1 - alpha) * weights
            reg_gradient = l1_reg + l2_reg
        

        # Combine the loss gradient and regularization gradient
        dw = loss_gradient + reg_gradient
...
def evaluate_model(weights, X, y, ymean, ystd):
    ...
    Args:
      ...
      y_mean : minimum value of target labels in the train set.
      y_std : maximum value of target labels in the train set.
    '''
...
def grid_search(X_train, y_train, X_dev, y_dev, y_train_mean, y_train_std, reg_strengths, regularizations, learning_rate=0.08, max_epochs=200, batch_size=None):
    ...
    for reg in regularizations:
        for reg_strength in reg_strengths:
            ...
            y_rescaled=y_dev*y_train_std+y_train_mean
            # Calculate the RMSE loss on the development set
            dev_loss=evaluate_model(model.weights,X_dev,y_rescaled,y_train_mean, y_train_std)
            # dev_loss = model.compute_rmse_loss(X_dev, y_dev, model.weights)[0][0]
            dev_losses[reg].append(dev_loss)

            # Update best_dev_loss and best_params if needed
    ...
    for alpha in alphas:
        for reg_strength in lambdas:
            model = LinearRegressionBatchGD(
                ...
                reg_strength=(reg_strength, alpha)
            )
            model.fit(X_train, y_train, X_dev, y_dev)
            val_loss = model.compute_rmse_loss(
                X_dev, y_dev, model.weights)
...
def get_two_way_interactions(X):
    ...
    X_extended = np.hstack((X, unique_interactions))
    return X_extended
...
def plot_reg_strength_vs_dev_loss(reg_strengths, dev_losses, regularizations):
    ...
    for reg in regularizations:
        plt.plot(reg_strengths, dev_losses[reg], marker='o',
                 label=f"Regularization: {reg.upper()}")
...
def save_prediction(Xmean, Xstd, ymean, ystd, weights, extend=0):
    ...
    pd.DataFrame(y_pred_hidden_test, columns=['Year']).to_csv(
        'roll_number.csv', index=True, header=True, index_label="ID")
...
if __name__ == '__main__':
    ...
    reg_strengths = [0.000001,0.00001,0.0001]
    ...
    # dev_losses, best_dev_loss, best_params = grid_search(X_train, y_train, X_dev, y_dev, y_train_mean, y_train_std,
    #                                                      reg_strengths, regularizations, learning_rate, max_epochs, batch_size)
    # print(best_params)
    # plot_reg_strength_vs_dev_loss(reg_strengths, dev_losses, regularizations)

    # best_model = LinearRegressionBatchGD(learning_rate=learning_rate, max_epochs=max_epochs, batch_size=batch_size,
    ...
    #     best_model.weights, X_test, y_test, y_train_mean, y_train_std)
    ...
    X_train_extended, y_train, X_dev_extended, y_dev, X_mean_extended, X_std_extended, y_train_mean, y_train_std = load_train_dev_dataset(
        1)
    X_test_extended, y_test = load_test_dataset(
        X_mean_extended, X_std_extended, 1)

    model = LinearRegressionBatchGD(learning_rate=learning_rate, max_epochs=max_epochs,
                                    batch_size=batch_size, regularization='l1', reg_strength=0.001)
    model.fit(X_train_extended, y_train, X_dev_extended, y_dev)

    test_loss = evaluate_model(
        model.weights, X_test_extended, y_test, y_train_mean, y_train_std)
    ...
    # dev_losses = grid_search_ElasticNet(X_train, y_train, X_dev, y_dev, y_train_mean, y_train_std,
                                    #    lambdas, alphas, learning_rate, max_epochs, batch_size)

    # print("Minimum dev loss:", min(dev_losses[2]))

    # -----------------------   Kaggle -----------------------------
    # Save predictions for the best model you get.
    save_prediction(X_mean_extended, X_std_extended,
                    y_train_mean, y_train_std, model.weights, 1)
